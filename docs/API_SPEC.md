...
受注管理 Phase 3では、出荷作成、出荷確定、出荷取消をSupabase RPCで実行する。フロントから在庫ロット、引当、受注明細、移動履歴へ複数UPDATEを直接発行しない。

- `generate_shipment_number`: `SHP-YYYYMMDD-000001` 形式でユーザーごとの出荷番号を採番する。
- `create_sales_order_shipment`: 受注と引当済ロットを基に出荷予定を作成する。全量出荷、一部出荷、ロット指定に対応する。
- `update_sales_order_shipment_status`: `Picking`、`Ready`、`Shipped`、`Cancelled` へ状態変更する。`Shipped` 確定時のみ在庫を減算し、`shipment` の移動履歴を残す。
- `refresh_sales_order_shipment_status`: 出荷済数量から受注と受注明細の `shipment_status`、`shipped_quantity`、`remaining_quantity` を再集計する。

出荷取消時は出荷履歴を削除せず、必要な数量を戻したうえで `inventory_movements.movement_type = shipment_cancel` を記録する。RPCは `authenticated` のみ実行可能とし、`auth.uid() = user_id` の範囲で処理する。
## 納品書RPC / PDF API方針

- `create_delivery_note_from_shipment`: 出荷済データから納品書と納品書明細をTransaction内で作成する。
- `generate_delivery_note_number`: `DN-YYYYMMDD-000001` 形式でユーザーごとの納品書番号を採番する。
- 納品書PDFは `deliveryNotePdfService` に分離し、UIコンポーネントにPDF内部処理を直書きしない。
- PDF本体は Supabase Storage `app-attachments` に保存し、DBにはURLとファイルメタ情報のみ保存する。
- RLSは `authenticated` のみ、`auth.uid() = user_id` のデータに限定する。
