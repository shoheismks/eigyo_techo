...
- 入力: 見積番号、提出日、有効期限（日付または自由入力）、通貨、合計金額、粗利率、ステータス、見積ファイル、メモ、失注理由。
...
- ステータス: 実装済
- 目的: 受注と在庫ロットを連携し、受注明細単位で在庫引当を行う。
- 画面: 受注一覧、受注作成/詳細、受注詳細内の在庫状況カード、在庫引当セクション。
- 入力: 優先度、受注明細、引当数量、ロット指定。
- 出力: 引当状況、引当率、受注数量、引当済、未引当、使用可能在庫、不足数量。
- 保存先: `sales_orders`, `sales_order_lines`, `inventory_reservations`, `inventory_movements`, `inventory_lots`
- 関連データ: 商品、在庫ロット、顧客、案件、見積、成約確認書。
- 拡張: 出荷、納品書、請求連携、帳票センター。
- 詳細: `docs/SALES_ORDER_PHASE2.md`
...
- 入力: 顧客、担当者、案件名、商品明細、数量、単位、販売単価、利用在庫、運賃、値引、支払条件、納品条件、有効期限（日付または「在庫限り」「相場変動時まで」などの自由入力）、備考。
...
- 帳票: 見積・受注作成時に企業グループ内の請求先／納品先を選択でき、名称・住所・担当者をsnapshot保存する。見積から受注作成する場合はsnapshotを引き継ぐ。
- 今後の拡張: 支社作成時の法人情報コピー範囲の詳細設定、企業グループ単位の分析レポート。
