...
- ステータス: 実装済
- 目的: フォロー、商談、見積、サンプル、クレーム期限を日付で確認する。
- 画面: `Calendar`
- 入力: 表示モード、対象日、各データの日付項目。
- 出力: 月表示、週表示、リスト表示、関連顧客カルテへの遷移。
- 保存先: なし。各関連データから予定を生成する。
- 関連データ: `customers`, `deal_histories`, `quotes`, `samples`, `claims`
- 今後の拡張: `calendar_events` テーブル、外部カレンダー連携、リマインド設定。
