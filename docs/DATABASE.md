...
- 目的: 出荷済データから作成した納品書ヘッダーを保存する。
- 主キー: `id`
- 主要カラム: `delivery_note_number`, `shipment_id`, `sales_order_id`, `customer_id`, `issue_date`, `delivery_date`, `price_visible`, `status`, `snapshot`, `delivery_note_pdf_url`, `delivery_note_pdf_file_name`, `delivery_note_pdf_storage_path`, `delivery_note_pdf_generated_at`, `delivery_note_pdf_history`, `is_deleted`
- 外部キー: `shipment_id -> shipments.id`, `sales_order_id -> sales_orders.id`
- 関連テーブル: `delivery_note_lines`, `shipments`, `sales_orders`, `customers`
- Storage利用: あり。PDFは `app-attachments` に保存し、DBにはURLとメタ情報のみ保存する。
- RLS: あり。`auth.uid() = user_id`
- 検索対象項目: `delivery_note_number`, 顧客名snapshot, 出荷番号snapshot, 受注番号snapshot
- 今後追加予定項目: 納品書改訂理由、送付履歴、外部帳票連携ID

### delivery_note_lines

- 目的: 納品書明細。出荷明細から商品、ロット、賞味期限、数量、価格表示用金額をsnapshot保存する。
- 主キー: `id`
- 主要カラム: `delivery_note_id`, `shipment_line_id`, `product_id`, `product_snapshot`, `quantity`, `unit`, `unit_price`, `amount`, `tax_rate`, `tax_amount`, `lot_snapshot`, `expiry_snapshot`
- 外部キー: `delivery_note_id -> delivery_notes.id`, `shipment_line_id -> shipment_lines.id`
- 関連テーブル: `delivery_notes`, `shipment_lines`, `products`
- Storage利用: なし。
- RLS: あり。`auth.uid() = user_id`
- 検索対象項目: `product_snapshot`, `lot_snapshot`, `expiry_snapshot`
- 今後追加予定項目: 検品数量、欠品理由、納品差異メモ
# 追記: 本社・支社／支店管理

## customers 追加カラム

- `parent_customer_id`: 本社の `customers.id`。本社は `null`。
- `office_type`: `head_office`, `branch`, `sales_office`, `store`, `factory`, `warehouse`, `other`。
- `branch_name`: 支社名、支店名、営業所名。
- `branch_code`: 任意の拠点コード。空欄可、入力時は同一ユーザー内で大文字小文字を区別せず一意。
- `is_head_office`: 本社かどうか。
- `billing_customer_id`: 既定請求先の `customers.id`。
- `shipping_customer_id`: 標準納品先の `customers.id`。

## 制約・RLS

- 既存RLSの `auth.uid() = user_id` を維持する。
- `parent_customer_id`, `billing_customer_id`, `shipping_customer_id` は同一 `user_id` の取引先だけ指定可能。
- `parent_customer_id` は本社レコードのみ指定可能。
- 多階層構造と循環参照は禁止する。
- 配下拠点がある本社は削除前に親子関係を解除または付け替える。

## 取引データとの関係

- 商談、見積、受注、出荷、納品書の `customer_id` は実際の取引拠点を維持する。
- 本社カルテの集計は、表示時に `本社ID + 配下拠点ID` を対象に集計する。
- 帳票は作成時点の取引先拠点、請求先、納品先をsnapshot保存する。
- `quotes` は `transaction_customer_snapshot`, `billing_customer_snapshot`, `shipping_customer_snapshot` を保持する。
- `sales_orders` は `customer_snapshot`, `billing_customer_snapshot`, `shipping_customer_snapshot` を保持し、見積から作成した場合は見積のsnapshotを引き継ぐ。

## 追加: 商品ブランド管理

### brands

- 目的: メーカー／仕入先とは別に、商品ブランドをユーザー単位で管理する。
- 主キー: `id`
- 主要カラム: `user_id`, `name`, `manufacturer_id`, `supplier_id`, `country`, `description`, `website_url`, `logo_url`, `is_active`, `deleted_at`, `created_at`, `updated_at`
- 関連テーブル: `products`, `suppliers`
- Storage利用: `logo_url` は将来 `app-attachments` または専用パスへ移行可能。現時点はURL参照のみ。
- RLS: あり。`auth.uid() = user_id`
- 検索対象: `name`, `country`, `description`
- 制約: 同一ユーザー内で `lower(name)` の重複不可。`deleted_at is null` の有効ブランドのみ対象。

### products ブランド追加カラム

- `brand_id`: `brands.id` を参照する任意項目。
- `brand_name`: 商品表示用のブランド名。ブランド未設定の場合は空欄可。
- 商品とブランドの内部紐付けは `brand_id`、帳票・過去文書の表示固定は明細 snapshot の `brandName` を使用する。

### 帳票 snapshot

- `quotes.quote_lines[]`: `brandId`, `brandName` を保存。
- `sales_order_lines`: `brand_id`, `brand_name` を保存。
- `invoice_lines` / `invoices.invoice_lines[]`: `brandId`, `brandName` を保存。
- `delivery_note_lines.product_snapshot`: `brandId`, `brandName` を保存。
- 商品マスターやブランド名を後で変更しても、過去の見積書・受注・請求書・納品書の表示は変更しない。
## 契約残管理

- 目的: 成約・受注した数量に対して、出荷済数量、出荷予定数量、契約残数量、契約残金額を再計算して確認する。
- 保存方針: 契約残の集計値はDBへ固定保存しない。`sales_orders`, `sales_order_lines`, `shipments`, `shipment_lines` から都度計算する。
- 追加View: `sales_order_contract_balance`
- 主な項目: `sales_order_id`, `sales_order_line_id`, `ordered_quantity`, `shipped_quantity`, `scheduled_quantity`, `remaining_quantity`, `practical_remaining_quantity`, `unit_price`, `ordered_amount`, `shipped_amount`, `scheduled_amount`, `remaining_amount`, `progress_rate`, `is_overdue`
- 出荷済数量: `shipments.status = 'Shipped'` の `shipment_lines.quantity` のみ集計する。
- 出荷予定数量: `shipments.status not in ('Shipped', 'Cancelled')` の `shipment_lines.quantity` を集計する。
- 契約残数量: `ordered_quantity - shipped_quantity`
- 実質残数量: `remaining_quantity - scheduled_quantity`
- 契約残金額: `remaining_quantity * unit_price`
- RLS: viewは `security_invoker = true` とし、元テーブルの `auth.uid() = user_id` を継承する。
- Backup / Restore: 契約残の集計値は保存せず、復元後に受注・出荷データから再計算する。
