...
現在のアプリは `customers`, `products`, `contacts`, `suppliers`, `business_cards`, `complaints`, `samples`, `attachments`, `mail_drafts` テーブルを使用します。Supabase SQL Editorで [supabase/customers.sql](supabase/customers.sql) を実行してください。
