...
現在のアプリは `customers` テーブルを使用します。Supabase SQL Editorで [supabase/customers.sql](supabase/customers.sql) を実行してください。

`Could not find the table 'public.customers' in the schema cache` が出る場合は、このテーブルがまだ作成されていません。
