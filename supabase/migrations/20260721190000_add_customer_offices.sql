-- Add head office / branch office management to customers.
-- Customers remain independent transaction records; parent links are only for grouping.
...
begin
  ...
  if new.billing_customer_id is not null and new.billing_customer_id <> '' then
  ...
  if new.shipping_customer_id is not null and new.shipping_customer_id <> '' then
    select count(*) into related_count
    from public.customers
