...
function toSupabaseRow(customer) {
  return {
    ...
    parent_customer_id: customer.parentCustomerId || null,
    office_type: customer.officeType || 'head_office',
    branch_name: customer.branchName,
    branch_code: customer.branchCode,
    is_head_office: customer.officeType ? customer.officeType === 'head_office' : Boolean(customer.isHeadOffice ?? true),
    billing_customer_id: customer.billingCustomerId || null,
    shipping_customer_id: customer.shippingCustomerId || null,
...
function fromSupabaseRow(row) {
  return {
    ...
    parentCustomerId: row.parent_customer_id ?? row.parentCustomerId ?? '',
    officeType: row.office_type ?? row.officeType ?? 'head_office',
    branchName: row.branch_name ?? row.branchName ?? '',
    branchCode: row.branch_code ?? row.branchCode ?? '',
    isHeadOffice: Boolean(row.is_head_office ?? row.isHeadOffice ?? ((row.office_type ?? row.officeType ?? 'head_office') === 'head_office')),
    billingCustomerId: row.billing_customer_id ?? row.billingCustomerId ?? '',
    shippingCustomerId: row.shipping_customer_id ?? row.shippingCustomerId ?? '',
