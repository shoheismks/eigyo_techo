...
function normalizeLine(line = {}, index = 0, userId = '', orderId = '') {
  ...
  return {
    ...
    originalUnitPrice: line.originalUnitPrice ?? line.original_unit_price ?? '',
    priceSource: line.priceSource ?? line.price_source ?? '',
    priceType: line.priceType ?? line.price_type ?? '',
    priceMasterId: line.priceMasterId ?? line.price_master_id ?? '',
    priceUnit: line.priceUnit ?? line.price_unit ?? line.unit ?? '',
    priceValidFrom: line.priceValidFrom ?? line.price_valid_from ?? '',
    priceValidTo: line.priceValidTo ?? line.price_valid_to ?? '',
    priceMatchedRule: line.priceMatchedRule ?? line.price_matched_rule ?? '',
    priceWarning: line.priceWarning ?? line.price_warning ?? '',
    isManualPrice: Boolean(line.isManualPrice ?? line.is_manual_price ?? false),
    priceOverrideReason: line.priceOverrideReason ?? line.price_override_reason ?? '',
    priceOverriddenAt: line.priceOverriddenAt ?? line.price_overridden_at ?? '',
    ...
    shippedQuantity: line.shippedQuantity ?? line.shipped_quantity ?? 0,
    remainingQuantity: line.remainingQuantity ?? line.remaining_quantity ?? Math.max(0, quantity - numberOrZero(line.shippedQuantity ?? line.shipped_quantity)),
    shipmentStatus: line.shipmentStatus ?? line.shipment_status ?? 'unshipped',
    memo: line.memo ?? '',
...
export function normalizeSalesOrder(order = {}, userId = '') {
  ...
  return {
    ...
    shipmentStatus: order.shipmentStatus ?? order.shipment_status ?? 'unshipped',
    shippedTotal: order.shippedTotal ?? order.shipped_total ?? 0,
...
function orderToRow(order) {
  return {
    ...
    shipment_status: order.shipmentStatus || 'unshipped',
    shipped_total: order.shippedTotal || 0,
    status: order.status,
...
function lineToRow(line, order) {
  return {
    ...
    original_unit_price: line.originalUnitPrice === '' ? null : Number(line.originalUnitPrice),
    price_source: line.priceSource || null,
    price_type: line.priceType || null,
    price_master_id: line.priceMasterId || null,
    price_unit: line.priceUnit || line.unit || null,
    price_valid_from: line.priceValidFrom || null,
    price_valid_to: line.priceValidTo || null,
    price_matched_rule: line.priceMatchedRule || null,
    price_warning: line.priceWarning || null,
    is_manual_price: Boolean(line.isManualPrice),
    price_override_reason: line.priceOverrideReason || null,
    price_overridden_at: line.priceOverriddenAt || null,
    ...
    remaining_quantity: line.remainingQuantity || 0,
    shipment_status: line.shipmentStatus || 'unshipped',
    memo: line.memo,
...
function rowToOrder(row, lines = [], history = [], userId = '') {
  return normalizeSalesOrder({
    ...
    shippedTotal: row.shipped_total,
    status: row.status,
...
function quoteLineToOrderLine(line = {}, index = 0, defaultTaxRate = DEFAULT_QUOTE_TAX_RATE) {
  ...
  return normalizeLine({
    ...
    priceMasterId: line.priceMasterId || '',
    priceUnit: line.priceUnit || line.unit || '',
    priceValidFrom: line.priceValidFrom || '',
    priceValidTo: line.priceValidTo || '',
    priceMatchedRule: line.priceMatchedRule || '',
    isManualPrice: Boolean(line.isManualPrice || false),
